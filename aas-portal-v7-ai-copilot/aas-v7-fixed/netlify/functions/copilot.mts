 ...
 * POST /api/copilot
 * 
 * PROTECTED ENDPOINT - Requires Auth0 JWT with Admin or Tech role
 *
...
// Allowed roles for copilot access
...
export default async (req: Request, _context: Context) => {
  ...
  // ============================================
  // AUTH CHECK - Require valid JWT with role
  // ============================================
  try {
    const payload = await verifyToken(req);
    const roles = getRoles(payload);
    
    // Check if user has Admin or Tech role
    if (!hasRole(payload, ALLOWED_ROLES)) {
      console.log(`[Copilot] Access denied for user ${payload.sub} with roles: ${roles.join(', ')}`);
      return new Response(
        JSON.stringify({ 
          error: "Forbidden", 
          message: "Copilot access requires Admin or Tech role",
          your_roles: roles 
        }),
    ...
    console.log(`[Copilot] Access granted for user ${payload.sub} with roles: ${roles.join(', ')}`);
